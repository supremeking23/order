...
<?php
	if(!isset($_POST['product_summary'])){
		...
		$query = "SELECT a.firstname,a.surname,a.customer_address,b.order_id,b.date_ordered,b.status,b.total_price FROM customertbl as a JOIN ordertbl as b ON(a.customer_id = b.customer_id) WHERE b.date_ordered BETWEEN '$from' AND '$to' AND status = 'pending'";
...
<?php
	...
	$wrapdf->Cell(0,10,"Pending Order Report",0,1,"C");
	$wrapdf->Cell(0,10,"From: ".$date_from." To: " .$date_to,0,1,"C");
	$wrapdf->Ln(8);


	$wrapdf->SetFont('Arial','B',10);
	$wrapdf->Cell(40,10,"Order Number",1,0,"C");
	$wrapdf->Cell(50,10,"Customer Name",1,0,"C");
	$wrapdf->Cell(80,10,"Customer Address",1,0,"C");
	$wrapdf->Cell(50,10,"Date Ordered",1,0,"C");
	...
	while($result_DisplayGenerate= mysqli_fetch_assoc($generatepdfQuery)){
		...
		*/
		...
		$pendingword="";
		if($result_DisplayGenerate['status']=="pending"){
			$pendingword="Pending";
		}

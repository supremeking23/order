...
<?php
	if(!isset($_POST['generate_summary'])){
		...
		$query = "SELECT a.firstname,a.surname,a.customer_address,b.order_id,b.date_ordered,b.date_approved,b.status,b.total_price FROM customertbl as a JOIN ordertbl as b ON(a.customer_id = b.customer_id) WHERE b.date_approved BETWEEN '$from' AND '$to' AND status = 'approve'";
...
<?php
	...
	$wrapdf->Cell(0,10,"Approve Order Report",0,1,"C");
	...
	$wrapdf->Cell(50,10,"Date Approved",1,0,"C");
	...
	while($result_DisplayGenerate= mysqli_fetch_assoc($generatepdfQuery)){
		...
		$c_width=20;
		$c_height=10;
		$wrapdf->vcell($c_width,$c_height,$x_axis,$result_DisplayGenerate['order_id'],1);
		*/
		...
		$c_width=50;
		$c_height=10;
		$wrapdf->vcell($c_width,$c_height,$x_axis,$result_DisplayGenerate['status'],1);

		*/

		$approvedword="";
		if($result_DisplayGenerate['status']=="approve"){
		...
		$wrapdf->Cell(50,10,$approvedword,1,0,"C");

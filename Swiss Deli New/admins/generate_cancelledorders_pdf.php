...
<?php
	if(!isset($_POST['product_summary'])){
		...
		//$asof_pendingorders = $_POST['asof'];

		$from = $_POST['from'];
		$to = $_POST['to'];

		$query = "SELECT a.firstname,a.surname,a.customer_address,b.order_id,b.date_ordered,b.status,b.total_price,b.date_cancelled FROM customertbl as a JOIN ordertbl as b ON(a.customer_id = b.customer_id) WHERE b.date_cancelled BETWEEN '$from' AND '$to' AND status = 'cancel'";
...
<?php
	...
	$wrapdf->Cell(0,10,"Cancel Order Report",0,1,"C");
	$wrapdf->Cell(0,10,"From: ".$date_from." To: " .$date_to,0,1,"C");
	$wrapdf->Ln(8);


	$wrapdf->SetFont('Arial','B',10);
	$wrapdf->Cell(40,10,"Order ID",1,0,"C");
	$wrapdf->Cell(50,10,"Customer Name",1,0,"C");
	$wrapdf->Cell(60,10,"Customer Address",1,0,"C");
	$wrapdf->Cell(40,10,"Date Ordered",1,0,"C");
	$wrapdf->Cell(40,10,"Date Cancelled",1,0,"C");
	$wrapdf->Cell(50,10,"Status",1,1,"C");
	...
	while($result_DisplayGenerate= mysqli_fetch_assoc($generatepdfQuery)){
		...
		$c_width=40;
		$c_height=10;
		$wrapdf->vcell($c_width,$c_height,$x_axis,"SD".$result_DisplayGenerate['order_id'],1,'C');
		*/

		$wrapdf->Cell(40,10,"SD".$result_DisplayGenerate['order_id'],1,0,"C");
		...
		$c_width=40;
		$c_height=10;
		$wrapdf->vcell($c_width,$c_height,$x_axis,$result_DisplayGenerate['date_cancelled'],1);
		...
		*/
		...
		$pendingword="";
		if($result_DisplayGenerate['status']=="cancel"){
			$pendingword="Cancelled";
		}




		$wrapdf->Cell(50,10,$pendingword,1,0,"C");
